...
async function calculateTotalPublicBallons(): Promise<number> {
  let totalAmount = 0;

  for (const color in BALLONS) {
    if (BALLONS[color].isPublic) {
      const amount = await fetchBallonAmount(BALLONS[color].id);
      totalAmount += amount;
    }
  }
...
calculateTotalPublicBallons().then((total) => {
  console.log(`Total amount of public ballons: ${total}`);
});
